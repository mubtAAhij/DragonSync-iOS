...
struct ServiceManagementView: View {
    ...
    enum ServiceAction: Identifiable {
        ...
        var title: String {
            switch self {
            case .toggle: return "String(localized: "toggle_service", comment: "Button text to toggle service on/off")"
            case .restart: return "String(localized: "restart_service", comment: "Button text to restart a service")"
            }
        }
    ...
    var body: some View {
        ...
        .alert("String(localized: "error", comment: "Generic error title")", isPresented: .init(
    ...
    private var healthStatusBar: some View {
        HStack(spacing: 12) {
            ...
            Text(viewModel.healthReport?.overallHealth.uppercased() ?? "String(localized: "no_connection", comment: "Status message when there is no connection")")
                .font(.appHeadline)
...
struct ServiceRowView: View {
    ...
    private var serviceDetails: some View {
        VStack(alignment: .leading, spacing: 8) {
            Group {
                detailRow("String(localized: "service_label", comment: "Label for service information")", service.service)
            ...
            HStack {
                ...
                Button {
                    ...
                    Label("String(localized: "restart", comment: "Button text to restart")", systemImage: "arrow.counterclockwise")
                        .font(.system(.caption, design: .monospaced))
                }
