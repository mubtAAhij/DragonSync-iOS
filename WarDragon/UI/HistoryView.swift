...
struct StoredEncountersView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .searchable(text: $searchText, prompt: String(localized: "search_by_id_or_caa_registration", comment: "Search prompt text"))
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        ...
                        Button {
                            ...
                            Label(String(localized: "export_csv", comment: "Button text for CSV export"), systemImage: "square.and.arrow.up")
                        }
                        Button(role: .destructive) {
                            showingDeleteConfirmation = true
                        } label: {
                            Label(String(localized: "delete_all", comment: "Button text for deleting all encounters"), systemImage: "trash")
                        }
                    } label: {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack {
                    ...
                    if let caaReg = encounter.metadata["caaRegistration"] {
                        Text(String(localized: "caa_registration", comment: "CAA registration label") + ": \(caaReg)")
                            .font(.appCaption)
                ...
                if let mac = encounter.metadata["mac"] {
                    Text(String(localized: "mac_address", comment: "MAC address label") + ": \(mac)")
                        .font(.appCaption)
                }
                
                HStack {
                    Label(String(localized: "flight_points", comment: "Flight path points count").replacingOccurrences(of: "{count}", with: "\(encounter.flightPath.count)"), systemImage: "map")
                ...
                Text(String(localized: "duration_label", comment: "Duration label") + ": \(formatDuration(encounter.totalFlightTime))")
                    .font(.appCaption)
                    .foregroundStyle(.secondary)
            }
            .padding(.vertical, 4)
...
//                  metadataSection // TODO metadata section
            ...
            .navigationTitle(String(localized: "encounter_details", comment: "Navigation title for encounter details"))
            .onAppear {
            ...
            .sheet(isPresented: $showingInfoEditor) {
                NavigationView {
                    DroneInfoEditor(droneId: encounter.id)
                        .navigationTitle(String(localized: "edit_drone_info", comment: "Navigation title for editing drone info"))
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        Picker(String(localized: "map_style", comment: "Picker label for map style"), selection: $selectedMapType) {
                            Text(String(localized: "map_standard", comment: "Standard map style option")).tag(MapStyle.standard)
                            Text(String(localized: "map_satellite", comment: "Satellite map style option")).tag(MapStyle.satellite)
                            Text(String(localized: "map_hybrid", comment: "Hybrid map style option")).tag(MapStyle.hybrid)
                        }
                        Button {
                            exportKML()
                        } label: {
                            Label(String(localized: "export_kml", comment: "Button text for KML export"), systemImage: "square.and.arrow.up")
                        }
                        Button(role: .destructive) {
                            showingDeleteConfirmation = true
                        } label: {
                ...
                if !encounter.flightPath.isEmpty {
                    ...
                    ForEach(encounter.flightPath.indices, id: \.self) { index in
                        ...
                        if point.isProximityPoint, let rssi = point.proximityRssi {
                            ...
                            Annotation(String(localized: "rssi_value", comment: "RSSI signal strength label") + ": \(Int(rssi)) dBm", coordinate: point.coordinate) {
                                VStack {
                                    Text(String(localized: "encrypted_drone", comment: "Label for encrypted drone"))
                                        .font(.caption)
                                    Text(String(localized: "radius_meters", comment: "Radius measurement label").replacingOccurrences(of: "{radius}", with: "\(Int(radius))"))
                                        .font(.caption)
                                        .foregroundColor(.primary)
                                }
                                .padding(6)
        ...
        private var encounterStats: some View {
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack {
                    VStack(alignment: .leading, spacing: 4) {
                        Text(String(localized: "first_detected", comment: "First detection label"))
                            .font(.appCaption)
                            .foregroundStyle(.secondary)
        ...
        private var macSectionHeader: some View {
            HStack {
                ...
                Text(String(localized: "device_using_mac_randomization", comment: "Description indicating device uses MAC randomization"))
                    .font(.appSubheadline)
                Spacer()
                Text(String(localized: "mac_addresses_count", comment: "Count of MAC addresses detected").replacingOccurrences(of: "{count}", with: "\(encounter.macHistory.count)"))
                    .font(.appCaption)
                    .foregroundColor(.secondary)
            }
        }
