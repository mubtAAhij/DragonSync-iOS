...
struct StoredEncountersView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .searchable(text: $searchText, prompt: String(localized: "search_by_id_or_caa_registration", comment: "Search prompt text for drone encounters"))
            .navigationTitle(String(localized: "encounter_history", comment: "Navigation title for encounter history view"))
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        Picker(String(localized: "sort_by", comment: "Sort menu label"), selection: $sortOrder) {
            ...
            .alert(String(localized: "delete_all_encounters", comment: "Alert title for deleting all encounters"), isPresented: $showingDeleteConfirmation) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack {
                    ...
                    if let caaReg = encounter.metadata["caaRegistration"] {
                        Text(String(localized: "caa_label", comment: "Label prefix for CAA registration") + ": \(caaReg)")
                            .font(.appCaption)
                ...
                if let mac = encounter.metadata["mac"] {
                    Text(String(localized: "mac_label", comment: "Label prefix for MAC address") + ": \(mac)")
                        .font(.appCaption)
                }
                
                HStack {
                    Label(String(localized: "flight_points", comment: "Label showing number of flight path points").replacingOccurrences(of: "%d", with: "\(encounter.flightPath.count)"), systemImage: "map")
                ...
                Text(String(localized: "duration_label", comment: "Label prefix for flight duration") + ": \(formatDuration(encounter.totalFlightTime))")
                    .font(.appCaption)
                    .foregroundStyle(.secondary)
            }
            .padding(.vertical, 4)
...
//                  metadataSection // TODO metadata section
            ...
            .sheet(isPresented: $showingInfoEditor) {
                NavigationView {
                    DroneInfoEditor(droneId: encounter.id)
                        .navigationTitle(String(localized: "edit_drone_info", comment: "Navigation title for drone info editor"))
                        .navigationBarTitleDisplayMode(.inline)
                        .toolbar {
                            ToolbarItem(placement: .navigationBarTrailing) {
                                Button(String(localized: "done", comment: "Done button to close drone info editor")) {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        ...
                        Button(role: .destructive) {
                            ...
                            Label(String(localized: "delete", comment: "Delete menu option"), systemImage: "trash")
                        }
                    } label: {
                        Image(systemName: "ellipsis.circle")
                    }
            ...
            .alert(String(localized: "delete_encounter", comment: "Alert title for deleting encounter"), isPresented: $showingDeleteConfirmation) {
                Button(String(localized: "delete", comment: "Delete button in encounter deletion dialog"), role: .destructive) {
                    storage.deleteEncounter(id: encounter.id)
                    dismiss() // Add this to return to list after deletion
                }
                Button(String(localized: "cancel", comment: "Cancel button in encounter deletion dialog"), role: .cancel) {}
            } message: {
            ...
            Map(position: $mapCameraPosition) {
                if !encounter.flightPath.isEmpty {
                    ...
                    if let start = normalPoints.first {
                        Annotation(String(localized: "start", comment: "Label for encounter start time"), coordinate: start.coordinate) {
                    ...
                    if let end = normalPoints.last, normalPoints.count > 1 {
                        Annotation(String(localized: "end", comment: "Label for encounter end time"), coordinate: end.coordinate) {
                    ...
                    ForEach(encounter.flightPath.indices, id: \.self) { index in
                        ...
                        if point.isProximityPoint, let rssi = point.proximityRssi {
                            ...
                            Annotation(String(localized: "rssi_label", comment: "Label prefix for RSSI value") + ": \(Int(rssi)) dBm", coordinate: point.coordinate) {
                                VStack {
                                    Text(String(localized: "encrypted_drone", comment: "Label for encrypted drone"))
                                        .font(.caption)
                                    Text(String(localized: "radius_label", comment: "Label for radius measurement").replacingOccurrences(of: "%d", with: "\(Int(radius))"))
                                        .font(.caption)
                                        .foregroundColor(.primary)
                                }
                                .padding(6)
                ...
                if let pilotLatStr = encounter.metadata["pilotLat"],
                   ...
                   pilotLat != 0 || pilotLon != 0 {
                    ...
                    Annotation(String(localized: "pilot", comment: "Label for pilot information"), coordinate: pilotCoordinate) {
        ...
        private var encounterStats: some View {
            VStack(alignment: .leading, spacing: 8) {
                Text(String(localized: "encounter_stats", comment: "Section header for encounter statistics"))
                    .font(.appHeadline)
                    .frame(maxWidth: .infinity, alignment: .center)
                
                StatsGrid {
                    StatItem(title: String(localized: "duration", comment: "Stats label for duration"), value: formatDuration(encounter.totalFlightTime))
                    StatItem(title: String(localized: "max_alt", comment: "Stats label for maximum altitude"), value: String(format: "%.1fm", encounter.maxAltitude))
                    StatItem(title: String(localized: "max_speed_stats", comment: "Stats label for maximum speed"), value: String(format: "%.1fm/s", encounter.maxSpeed))
                    StatItem(title: String(localized: "avg_rssi", comment: "Stats label for average RSSI"), value: String(format: "%.1fdBm", encounter.averageRSSI))
                    StatItem(title: String(localized: "points", comment: "Stats label for number of points"), value: "\(encounter.flightPath.count)")
                    StatItem(title: String(localized: "signatures", comment: "Stats label for number of signatures"), value: "\(encounter.signatures.count)")
                }
                Text(String(localized: "encounter_timeline", comment: "Section header for encounter timeline"))
                    .font(.appHeadline)
                    .frame(maxWidth: .infinity, alignment: .center)
                ...
                HStack {
                    ...
                    VStack(alignment: .trailing, spacing: 4) {
                        Text(String(localized: "last_contact", comment: "Label for last contact time"))
                            .font(.appCaption)
                            .foregroundStyle(.secondary)
        ...
        private var macSectionTitle: some View {
            Text(String(localized: "mac_randomization", comment: "Section header for MAC randomization information"))
                .font(.appHeadline)
                .frame(maxWidth: .infinity, alignment: .center)
        }
        ...
        private var macSectionHeader: some View {
            HStack {
                ...
                Text("String(localized: "device_using_mac_randomization", comment: "Warning message indicating device is using MAC address randomization")")
                    .font(.appSubheadline)
                Spacer()
                Text(String(localized: "mac_addresses_count", comment: "Shows count of MAC addresses").replacingOccurrences(of: "{count}", with: "\(encounter.macHistory.count)"))
                    .font(.appCaption)
                    .foregroundColor(.secondary)
            }
        }
