...
struct StoredEncountersView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .searchable(text: $searchText, prompt: String(localized: "search_by_id_or_caa_registration", comment: "Search placeholder text for drone encounters"))
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        ...
                        Button {
                            ...
                            Label(String(localized: "export_csv", comment: "Menu item for exporting data as CSV"), systemImage: "square.and.arrow.up")
                        }
                        Button(role: .destructive) {
                            showingDeleteConfirmation = true
                        } label: {
                            Label(String(localized: "delete_all", comment: "Menu item for deleting all encounters"), systemImage: "trash")
                        }
                    } label: {
    ...
    struct EncounterRow: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack {
                    Label(String(localized: "points_count", comment: "Label for number of flight path points").replacingOccurrences(of: "%d", with: "\(encounter.flightPath.count)"), systemImage: "map")
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                if !encounter.customName.isEmpty {
                                    ...
                                    Text(String(localized: "unnamed_drone", comment: "Default name for drones without custom names"))
                                        .font(.system(.title2, design: .monospaced))
                                        .foregroundColor(.secondary)
                                }
...
//                  metadataSection // TODO metadata section
            ...
            .navigationTitle(String(localized: "encounter_details", comment: "Navigation title for encounter details view"))
            .onAppear {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        Picker(String(localized: "map_style", comment: "Picker label for map style selection"), selection: $selectedMapType) {
                        ...
                        Button {
                            ...
                            Label(String(localized: "export_kml", comment: "Menu item for exporting data as KML"), systemImage: "square.and.arrow.up")
                        }
                        Button(role: .destructive) {
                            showingDeleteConfirmation = true
                        } label: {
            ...
            .alert(String(localized: "delete_encounter", comment: "Alert title for deleting single encounter"), isPresented: $showingDeleteConfirmation) {
                Button(String(localized: "delete", comment: "Delete button text"), role: .destructive) {
                ...
                Text(String(localized: "delete_encounter_confirmation", comment: "Confirmation message for deleting an encounter"))
            }
        }
            ...
            Map(position: $mapCameraPosition) {
                if !encounter.flightPath.isEmpty {
                    ...
                    if let start = normalPoints.first {
                        Annotation(String(localized: "start", comment: "Label for start time or location"), coordinate: start.coordinate) {
                    ...
                    if let end = normalPoints.last, normalPoints.count > 1 {
                        Annotation(String(localized: "end", comment: "Label for end time or location"), coordinate: end.coordinate) {
                    ...
                    ForEach(encounter.flightPath.indices, id: \.self) { index in
                        ...
                        if point.isProximityPoint, let rssi = point.proximityRssi {
                            ...
                            Annotation(String(localized: "rssi_label", comment: "Label for RSSI signal strength") + ": \(Int(rssi)) dBm", coordinate: point.coordinate) {
                                VStack {
        ...
        private var encounterStats: some View {
            VStack(alignment: .leading, spacing: 8) {
                ...
                StatsGrid {
                    StatItem(title: String(localized: "duration", comment: "Duration label in statistics"), value: formatDuration(encounter.totalFlightTime))
                    StatItem(title: String(localized: "max_alt", comment: "Maximum altitude label in statistics"), value: String(format: "%.1fm", encounter.maxAltitude))
                    StatItem(title: String(localized: "max_speed_stats", comment: "Maximum speed label in statistics"), value: String(format: "%.1fm/s", encounter.maxSpeed))
                    StatItem(title: String(localized: "avg_rssi", comment: "Average RSSI label in statistics"), value: String(format: "%.1fdBm", encounter.averageRSSI))
                    StatItem(title: String(localized: "points", comment: "Points label in statistics"), value: "\(encounter.flightPath.count)")
                    StatItem(title: String(localized: "signatures", comment: "Signatures label in statistics"), value: "\(encounter.signatures.count)")
                }
                ...
                HStack {
                    ...
                    VStack(alignment: .trailing, spacing: 4) {
                        Text(String(localized: "last_contact", comment: "Label showing when drone was last detected"))
                            .font(.appCaption)
                            .foregroundStyle(.secondary)
        ...
        private var macSectionTitle: some View {
            Text(String(localized: "mac_randomization", comment: "Section header for MAC address randomization info"))
                .font(.appHeadline)
                .frame(maxWidth: .infinity, alignment: .center)
        }
        ...
        private var macSectionHeader: some View {
            HStack {
                ...
                Text(String(localized: "device_using_mac_randomization", comment: "Status text indicating device is randomizing MAC addresses"))
                    .font(.appSubheadline)
                Spacer()
        ...
        private var flightDataSection: some View {
            VStack(alignment: .leading, spacing: 8) {
                Text(String(localized: "flight_data", comment: "Section header for flight information"))
                    .font(.appHeadline)
                    .frame(maxWidth: .infinity, alignment: .center)
       ...
       var body: some View {
           VStack {
               ...
               if let minValue = data.min(), let maxValue = data.max(), data.count > 1 {
                   ...
                   Text(String(localized: "insufficient_data", comment: "Message shown when not enough data is available"))
                       .font(.appCaption)
                       .foregroundColor(.secondary)
               }
           }
