...
struct DroneDetailView: View {
           ...
           VStack(spacing: 8) {
               HStack {
                   ...
                   Button(action: {
                       ...
                       Text(showAllLocations ? String(localized: "drone_only", comment: "Button text to show only drone on map") : String(localized: "show_all", comment: "Button text to show all locations on map"))
                           .font(.caption)
               ...
               Map(position: $mapCameraPosition) {
                   ...
                   if message.pilotLat != "0.0" && message.pilotLon != "0.0",
                      ...
                      let pilotLon = Double(message.pilotLon) {
                       ...
                       Annotation(String(localized: "pilot", comment: "Map annotation label for pilot"), coordinate: pilotCoordinate) {
    ...
    private var flightPathStatsSection: some View {
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                DroneInfoRow(title: String(localized: "total_points", comment: "Label for total points field"), value: "\(flightPath.count)")
                
                if flightPath.count > 1 {
                    let distance = calculateTotalDistance()
                    DroneInfoRow(title: String(localized: "total_distance", comment: "Label for total distance field"), value: String(format: String(localized: "distance_meters", comment: "Distance value in meters").replacingOccurrences(of: "{value}", with: String(format: "%.1f", distance)), distance))
                    
                    if let bounds = calculateFlightBounds() {
                        DroneInfoRow(title: String(localized: "area_covered", comment: "Label for area covered field"), value: String(format: String(localized: "area_degrees", comment: "Area coverage in degrees").replacingOccurrences(of: "{lat}", with: String(format: "%.3f", bounds.latSpan)).replacingOccurrences(of: "{lon}", with: String(format: "%.3f", bounds.lonSpan)), bounds.latSpan, bounds.lonSpan))
                    }
                }
            }
            .padding()
