...
struct DroneDetailView: View {
           ...
           VStack(spacing: 8) {
               ...
               Map(position: $mapCameraPosition) {
                   ...
                   if let droneCoordinate = message.coordinate {
                       Annotation(String(localized: "drone", comment: "Map annotation for drone position"), coordinate: droneCoordinate) {
                   ...
                   if message.pilotLat != "0.0" && message.pilotLon != "0.0",
                      ...
                      let pilotLon = Double(message.pilotLon) {
                       ...
                       Annotation(String(localized: "pilot", comment: "Map annotation for pilot position"), coordinate: pilotCoordinate) {
                   ...
                   if let startPoint = flightPath.first, flightPath.count > 1 {
                       Annotation(String(localized: "start", comment: "Map annotation for flight path start"), coordinate: startPoint) {
                   ...
                   if let endPoint = flightPath.last,
                      ...
                      !(endPoint.latitude == startPoint.latitude && endPoint.longitude == startPoint.longitude) {
                       Annotation(String(localized: "latest", comment: "Map annotation for latest position"), coordinate: endPoint) {
    ...
    private var droneInfoSection: some View {
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                DroneInfoRow(title: String(localized: "type", comment: "Label for drone type"), value: message.type)
                DroneInfoRow(title: "ID", value: message.id)
                DroneInfoRow(title: String(localized: "id_type", comment: "Label for ID type"), value: message.idType)
                if !message.description.isEmpty {
                    DroneInfoRow(title: String(localized: "description", comment: "Label for drone description"), value: message.description)
                }
                if !message.selfIDText.isEmpty {
                    DroneInfoRow(title: String(localized: "self_id", comment: "Label for self-ID"), value: message.selfIDText)
                }
                if let manufacturer = message.manufacturer {
                    DroneInfoRow(title: String(localized: "manufacturer", comment: "Label for drone manufacturer"), value: manufacturer)
                }
            }
            .padding()
            .background(
    ...
    private var locationDetailsSection: some View {
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                // Current position
                DroneInfoRow(title: String(localized: "current_position", comment: "Label for current position"), value: "\(message.lat), \(message.lon)")
                DroneInfoRow(title: String(localized: "altitude", comment: "Label for altitude"), value: String(localized: "meters_unit", comment: "Meters unit display").replacingOccurrences(of: "{value}", with: "\(message.alt)"))
                if let height = message.height {
                    DroneInfoRow(title: String(localized: "height_agl", comment: "Label for height above ground level"), value: String(localized: "meters_unit", comment: "Meters unit display").replacingOccurrences(of: "{value}", with: "\(height)"))
                }
                if message.speed != "" {
                    DroneInfoRow(title: String(localized: "speed", comment: "Label for speed"), value: String(localized: "meters_per_second", comment: "Meters per second unit display").replacingOccurrences(of: "{value}", with: "\(message.speed)"))
                }
                
                DroneInfoRow(title: String(localized: "vertical_speed", comment: "Label for vertical speed"), value: String(localized: "meters_per_second", comment: "Meters per second unit display").replacingOccurrences(of: "{value}", with: "\(message.vspeed)"))
                
                // Track data from CoT messages
                let trackData = message.getTrackData()
                if let course = trackData.course, course != "0.0" && !course.isEmpty {
                    DroneInfoRow(title: String(localized: "course", comment: "Label for course direction"), value: String(localized: "degrees_unit", comment: "Degrees unit display").replacingOccurrences(of: "{value}", with: "\(course)"))
                }
                if let speed = trackData.speed, speed != "0.0" {
                    DroneInfoRow(title: String(localized: "track_speed", comment: "Label for track speed"), value: String(localized: "meters_per_second", comment: "Meters per second unit display").replacingOccurrences(of: "{value}", with: "\(speed)"))
                }
                if let bearing = trackData.bearing, !bearing.isEmpty {
                    DroneInfoRow(title: String(localized: "bearing", comment: "Label for bearing direction"), value: "\(bearing)°")
                }
                ...
                // Home location
                if message.homeLat != "0.0" && message.homeLon != "0.0" {
                    DroneInfoRow(title: String(localized: "home_location", comment: "Label for home location"), value: "\(message.homeLat), \(message.homeLon)")
                }
        ...
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                ...
                if flightPath.count > 1 {
                    ...
                    DroneInfoRow(title: String(localized: "total_distance", comment: "Label for total distance"), value: String(format: String(localized: "meters_decimal_unit", comment: "Meters unit with decimal display").replacingOccurrences(of: "{value}", with: String(format: "%.1f", distance)), distance))
                    
                    if let bounds = calculateFlightBounds() {
                        DroneInfoRow(title: String(localized: "area_covered", comment: "Label for area covered"), value: String(format: String(localized: "degrees_area_format", comment: "Format for area in degrees").replacingOccurrences(of: "{lat}", with: String(format: "%.3f", bounds.latSpan)).replacingOccurrences(of: "{lon}", with: String(format: "%.3f", bounds.lonSpan)), bounds.latSpan, bounds.lonSpan))
                    }
                }
            }
            .padding()
        ...
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                if let rssi = message.rssi {
                    DroneInfoRow(title: "RSSI", value: String(localized: "dbm_unit", comment: "dBm unit display").replacingOccurrences(of: "{value}", with: "\(rssi)"))
                }
                ...
                if let macs = cotViewModel.macIdHistory[message.uid], macs.count > 1 {
                    DroneInfoRow(title: String(localized: "mac_randomization", comment: "Label for MAC randomization"), value: String(localized: "mac_randomization_detected", comment: "MAC randomization detection message").replacingOccurrences(of: "{count}", with: "\(macs.count)"))
                }
                
                // Show signal sources if available
                if !message.signalSources.isEmpty {
                    DroneInfoRow(title: String(localized: "signal_sources", comment: "Label for signal sources"), value: "\(message.signalSources.count)")
                }
            }
            .padding()
            .background(
