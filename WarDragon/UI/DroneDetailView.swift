           ...
           VStack(spacing: 8) {
               HStack {
                   ...
                   Button(action: {
                       ...
                       Text(showAllLocations ? "String(localized: "drone_only", comment: "Button text to show only drone location on map")" : "String(localized: "show_all", comment: "Button text to show all locations on map")")
                           .font(.caption)
               ...
               Map(position: $mapCameraPosition) {
                   ...
                   if let droneCoordinate = message.coordinate {
                       Annotation("String(localized: "drone", comment: "Map annotation for drone location")", coordinate: droneCoordinate) {
                   ...
                   if message.pilotLat != "0.0" && message.pilotLon != "0.0",
                      ...
                      let pilotLon = Double(message.pilotLon) {
                       ...
                       Annotation("String(localized: "pilot", comment: "Map annotation for pilot location")", coordinate: pilotCoordinate) {
                   ...
                   if let startPoint = flightPath.first, flightPath.count > 1 {
                       Annotation("String(localized: "start", comment: "Map annotation for flight path start point")", coordinate: startPoint) {
                   ...
                   if let endPoint = flightPath.last,
                      ...
                      !(endPoint.latitude == startPoint.latitude && endPoint.longitude == startPoint.longitude) {
                       Annotation("String(localized: "latest", comment: "Map annotation for latest flight path point")", coordinate: endPoint) {
    ...
    private var locationDetailsSection: some View {
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                ...
                DroneInfoRow(title: "String(localized: "altitude", comment: "Label for altitude field")", value: String(localized: "meters_value", comment: "Value in meters").replacingOccurrences(of: "{value}", with: "\(message.alt)"))
                if let height = message.height {
                    DroneInfoRow(title: "String(localized: "height_agl", comment: "Label for height above ground level")", value: String(localized: "meters_value", comment: "Value in meters").replacingOccurrences(of: "{value}", with: "\(height)"))
                }
                if message.speed != "" {
                    DroneInfoRow(title: "String(localized: "speed", comment: "Label for speed field")", value: String(localized: "meters_per_second_value", comment: "Value in meters per second").replacingOccurrences(of: "{value}", with: "\(message.speed)"))
                }
                
                DroneInfoRow(title: "String(localized: "vertical_speed", comment: "Label for vertical speed field")", value: String(localized: "meters_per_second_value", comment: "Value in meters per second").replacingOccurrences(of: "{value}", with: "\(message.vspeed)"))
                
                // Track data from CoT messages
                let trackData = message.getTrackData()
                if let course = trackData.course, course != "0.0" && !course.isEmpty {
                    DroneInfoRow(title: "String(localized: "course", comment: "Label for course field")", value: String(localized: "degrees_value", comment: "Value in degrees").replacingOccurrences(of: "{value}", with: "\(course)"))
                }
                if let speed = trackData.speed, speed != "0.0" {
                    DroneInfoRow(title: "String(localized: "track_speed", comment: "Label for track speed field")", value: String(localized: "meters_per_second_value", comment: "Value in meters per second").replacingOccurrences(of: "{value}", with: "\(speed)"))
                }
        ...
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                ...
                if flightPath.count > 1 {
                    ...
                    DroneInfoRow(title: "String(localized: "total_distance", comment: "Label for total distance traveled")", value: String(format: String(localized: "meters_decimal_format", comment: "Decimal meters format").replacingOccurrences(of: "{value}", with: String(format: "%.1f", distance)), distance))
                    
                    if let bounds = calculateFlightBounds() {
                        DroneInfoRow(title: "String(localized: "area_covered", comment: "Label for area covered by flight path")", value: String(format: String(localized: "degrees_area_format", comment: "Degrees area format").replacingOccurrences(of: "{lat}", with: String(format: "%.3f", bounds.latSpan)).replacingOccurrences(of: "{lon}", with: String(format: "%.3f", bounds.lonSpan)), bounds.latSpan, bounds.lonSpan))
                    }
                }
            }
            .padding()
        ...
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                if let rssi = message.rssi {
                    DroneInfoRow(title: "RSSI", value: String(localized: "dbm_value", comment: "Value in dBm").replacingOccurrences(of: "{value}", with: "\(rssi)"))
                }
                ...
                if let macs = cotViewModel.macIdHistory[message.uid], macs.count > 1 {
                    DroneInfoRow(title: "String(localized: "mac_randomization", comment: "Label for MAC randomization detection")", value: String(localized: "detected_macs_count", comment: "Count of detected MAC addresses").replacingOccurrences(of: "{count}", with: "\(macs.count)"))
                }
