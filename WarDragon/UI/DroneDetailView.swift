    ...
    private var locationDetailsSection: some View {
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                ...
                DroneInfoRow(title: String(localized: "altitude", comment: "Label for altitude field"), value: String(localized: "altitude_meters", comment: "Altitude value in meters").replacingOccurrences(of: "{value}", with: "\(message.alt)"))
                if let height = message.height {
                    DroneInfoRow(title: String(localized: "height_agl", comment: "Label for height above ground level"), value: String(localized: "height_meters", comment: "Height value in meters").replacingOccurrences(of: "{value}", with: "\(height)"))
                }
                if message.speed != "" {
                    DroneInfoRow(title: String(localized: "speed", comment: "Label for speed field"), value: String(localized: "speed_meters_per_second", comment: "Speed value in meters per second").replacingOccurrences(of: "{value}", with: "\(message.speed)"))
                }
                
                DroneInfoRow(title: String(localized: "vertical_speed", comment: "Label for vertical speed field"), value: String(localized: "vertical_speed_meters_per_second", comment: "Vertical speed value in meters per second").replacingOccurrences(of: "{value}", with: "\(message.vspeed)"))
                
                // Track data from CoT messages
                let trackData = message.getTrackData()
                if let course = trackData.course, course != "0.0" && !course.isEmpty {
                    DroneInfoRow(title: String(localized: "course", comment: "Label for course field"), value: String(localized: "course_degrees", comment: "Course value in degrees").replacingOccurrences(of: "{value}", with: "\(course)"))
                }
                if let speed = trackData.speed, speed != "0.0" {
                    DroneInfoRow(title: String(localized: "track_speed", comment: "Label for track speed field"), value: String(localized: "track_speed_meters_per_second", comment: "Track speed value in meters per second").replacingOccurrences(of: "{value}", with: "\(speed)"))
                }
        ...
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                ...
                if flightPath.count > 1 {
                    ...
                    DroneInfoRow(title: String(localized: "total_distance", comment: "Label for total distance field"), value: String(format: String(localized: "distance_meters", comment: "Distance value in meters format").replacingOccurrences(of: "{value}", with: String(format: "%.1f", distance)), distance))
                    
                    if let bounds = calculateFlightBounds() {
                        DroneInfoRow(title: String(localized: "area_covered", comment: "Label for area covered field"), value: String(format: String(localized: "area_degrees_format", comment: "Area covered in degrees format").replacingOccurrences(of: "{lat}", with: String(format: "%.3f", bounds.latSpan)).replacingOccurrences(of: "{lon}", with: String(format: "%.3f", bounds.lonSpan)), bounds.latSpan, bounds.lonSpan))
                    }
                }
            }
            .padding()
        ...
        VStack(spacing: 8) {
            ...
            VStack(spacing: 4) {
                if let rssi = message.rssi {
                    DroneInfoRow(title: "RSSI", value: String(localized: "rssi_dbm", comment: "RSSI value in dBm").replacingOccurrences(of: "{value}", with: "\(rssi)"))
                }
                ...
                if let macs = cotViewModel.macIdHistory[message.uid], macs.count > 1 {
                    DroneInfoRow(title: String(localized: "mac_randomization", comment: "Label for MAC randomization field"), value: String(localized: "mac_randomization_detected", comment: "MAC randomization detected message").replacingOccurrences(of: "{count}", with: "\(macs.count)"))
                }
