            ...
            if let mac = mac, let remoteId = remoteId {
                Button(action: {
                    ...
                    HStack {
                        ...
                        Text(isLoading ? String(localized: "loading", comment: "Loading indicator text") : String(localized: "faa_lookup", comment: "Button text for FAA data lookup"))
                    }
                ...
                .alert(String(localized: "faa_lookup_error", comment: "Alert title for FAA lookup error"), isPresented: $showingError) {
                    Button(String(localized: "ok", comment: "OK button for alert"), role: .cancel) {}
                } message: {
                    Text(faaService.error ?? String(localized: "unknown_error_occurred", comment: "Generic error message"))
                }
            }
        }
