...
struct StatusListView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showServiceManagement) {
            NavigationView {
                ServiceManagementView(viewModel: serviceViewModel)
                    .navigationTitle(String(localized: "service_management", comment: "Title for service management view"))
        ...
        .alert(String(localized: "delete_message", comment: "Alert title for deleting message"), isPresented: $showingDeleteConfirmation) {
            ...
            Button(String(localized: "cancel", comment: "Cancel delete action"), role: .cancel) {
                messageToDelete = nil
            }
        } message: {
            Text(String(localized: "confirm_delete_status_message", comment: "Confirmation message for deleting status message"))
        }
    }
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            if let lastReceived = statusViewModel.lastStatusMessageReceived {
                ...
                HStack {
                    Text(String(localized: "no_status_messages", comment: "Message when no status messages received"))
                        .font(.system(.caption, design: .monospaced))
                        .foregroundColor(.secondary)
                    Spacer()
                }
        ...
        Button(action: { showServiceManagement = true }) {
            VStack(spacing: 4) {
                ...
                HStack(spacing: 12) {
                    ...
                    Text(healthReport?.overallHealth.uppercased() ?? String(localized: "no_connection", comment: "Status text when no connection"))
                        .font(.appHeadline)
                ...
                if !criticalServices.isEmpty {
                    HStack {
                        Text(String(localized: "critical_issues_count", comment: "Label showing critical issues count") + ": \(criticalServices.count))
                            .font(.appCaption)
                            .foregroundColor(.red)
                        Spacer()
                    }
