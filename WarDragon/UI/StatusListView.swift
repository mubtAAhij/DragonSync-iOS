...
struct StatusListView: View {
    ...
    var body: some View {
                            ...
                            StatusMessageView(message: message, statusViewModel: statusViewModel)
                                .swipeActions(edge: .trailing, allowsFullSwipe: true) {
                                    Button(role: .destructive) {
                                        ...
                                        Label(String(localized: "delete", comment: "Delete action label"), systemImage: "trash")
                                    }
                                }
                                .contextMenu {
                                    Button(role: .destructive, action: {
                                        ...
                                        Label(String(localized: "delete", comment: "Delete context menu action"), systemImage: "trash")
                                    }
                                }
                        }
        ...
        .navigationTitle(String(localized: "system_status", comment: "Navigation title for system status screen"))
        .sheet(isPresented: $showServiceManagement) {
            NavigationView {
                ServiceManagementView(viewModel: serviceViewModel)
                    .navigationTitle(String(localized: "service_management", comment: "Title for service management screen"))
        ...
        .alert(String(localized: "delete_message", comment: "Title for delete message confirmation dialog"), isPresented: $showingDeleteConfirmation) {
            ...
            Text(String(localized: "delete_message_confirmation", comment: "Confirmation message for deleting status message"))
        }
    }
        ...
        VStack(spacing: 12) {
            HStack {
                HStack(spacing: 8) {
                    ...
                    Text(String(localized: "system_connection", comment: "Header for system connection status"))
            ...
            if let lastReceived = statusViewModel.lastStatusMessageReceived {
                HStack {
                    Text(String(localized: "last_message_received", comment: "Label for last message received timestamp"))
        ...
        Button(action: { showServiceManagement = true }) {
            VStack(spacing: 4) {
                ...
                if !criticalServices.isEmpty {
                    HStack {
                        Text(String(localized: "critical_issues_count", comment: "Label showing number of critical issues") + ": \(criticalServices.count)")
                            .font(.appCaption)
                            .foregroundColor(.red)
                        Spacer()
                    }
