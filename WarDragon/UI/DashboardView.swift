        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let antStats = statusViewModel.statusMessages.last?.antStats {
                HStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading) {
                        Text(String(localized: "status_label", comment: "Status label in SDR card"))
                            .font(.appCaption)
                            .foregroundColor(.secondary)
                        Text(antStats.plutoTemp != 0.0 || antStats.zynqTemp != 0.0 ? String(localized: "active", comment: "Active status text") : String(localized: "inactive", comment: "Inactive status text"))
                           .font(.system(.caption, design: .monospaced))
...
struct WarningsCard: View {
    ...
    private var activeWarnings: [SystemWarning] {
        ...
        if !spoofedDrones.isEmpty {
            warnings.append(SystemWarning(
                id: "spoof",
                title: String(localized: "possible_spoofed_signals", comment: "Warning label for possible spoofed signals"),
