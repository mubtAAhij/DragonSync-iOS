...
struct SettingsView: View {
    ...
    var body: some View {
        Form {
            Section(String(localized: "connection", comment: "Connection settings section")) {
                HStack {
                ...
                Picker(String(localized: "mode", comment: "Connection mode setting"), selection: .init(
                ...
                if settings.connectionMode == .zmq {
                    HStack {
                        TextField(String(localized: "zmq_host", comment: "ZMQ host input field"), text: .init(
                    ...
                    HStack {
                        TextField(String(localized: "multicast_host", comment: "Multicast host input field"), text: .init(
                ...
                Toggle(isOn: .init(
                    ...
                    Text(settings.isListening && cotHandler.isListeningCot ? String(localized: "active", comment: "Active connection status") : String(localized: "inactive", comment: "Inactive connection status"))
                }
                .disabled(!settings.isHostConfigurationValid())
            }
            
            Section(String(localized: "preferences", comment: "Preferences settings section")) {
                Toggle(String(localized: "auto_spoof_detection", comment: "Auto spoof detection setting"), isOn: .init(
                    get: { settings.spoofDetectionEnabled },
                    set: { settings.spoofDetectionEnabled = $0 }
                ))
                
                Toggle(String(localized: "keep_screen_on", comment: "Keep screen on setting"), isOn: .init(
                    get: { settings.keepScreenOn },
                    set: { settings.updatePreferences(notifications: settings.notificationsEnabled, screenOn: $0) }
                ))
                
                Toggle(String(localized: "enable_background_detection", comment: "Enable background detection setting"), isOn: .init(
            ...
            Section(String(localized: "notifications", comment: "Notifications settings section")) {
                Toggle(String(localized: "enable_push_notifications", comment: "Enable push notifications setting"), isOn: .init(
            ...
            Section(String(localized: "webhooks_external_services", comment: "Webhooks and external services section")) {
                Toggle(String(localized: "enable_webhooks", comment: "Enable webhooks setting"), isOn: .init(
                    ...
                    NavigationLink(destination: WebhookSettingsView()) {
                        HStack {
                            ...
                            VStack(alignment: .leading) {
                                ...
                                Text(String(localized: "services_configured", comment: "Number of webhook services configured"))
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                            Spacer()
            ...
            Section(String(localized: "performance", comment: "Performance settings section")) {
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        ...
                        Stepper(value: $settings.messageProcessingInterval, in: 300...5000, step: 50) {
                            Text(String(localized: "milliseconds_unit", comment: "Milliseconds unit display"))
                                .font(.appCaption)
                                .bold()
            ...
            Section(String(localized: "ports", comment: "Ports settings section")) {
