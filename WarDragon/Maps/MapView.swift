        ...
        Map {
            ...
            if let ring = cotViewModel.alertRings.first(where: { $0.droneId == message.uid }),
               Double(message.lat) ?? 0 == 0 && Double(message.lon) ?? 0 == 0 {
                ...
                Marker(String(localized: "meters_unit", comment: "Meters measurement unit").replacingOccurrences(of: "{value}", with: "\(Int(ring.radius))"), coordinate: ring.centerCoordinate)
                    .tint(.yellow)
            }
        }
        .frame(height: 200)
