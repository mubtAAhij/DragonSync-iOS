        ...
        ZStack {
            Map(position: $mapCameraPosition) {
                ...
                ForEach(cotViewModel.alertRings, id: \.id) { ring in
                    ...
                    Annotation(String(localized: "rssi_value", comment: "RSSI signal strength display"), coordinate: ring.centerCoordinate) {
                        VStack {
            ...
            VStack {
                ...
                Button(action: { showDroneList.toggle() }) {
                    Text(String(localized: "drone_count", comment: "Number of drones detected"))
                        .padding()
                        .background(.ultraThinMaterial)
                        .cornerRadius(20)
                }
                ...
                List(uniqueDrones) { message in
                    NavigationLink(destination: DroneDetailView(message: message, flightPath: flightPaths[message.uid]?.map { $0.coordinate } ?? [], cotViewModel: cotViewModel)) {
                        VStack(alignment: .leading) {
                            Text(message.uid)
                                .font(.appHeadline)
                            Text(String(localized: "position_coordinates", comment: "Drone position coordinates"))
                                .font(.appCaption)
                            if !message.description.isEmpty {
                                Text(String(localized: "description_label", comment: "Description field label"))
                                    .font(.appCaption)
                            }
                            if message.pilotLat != "0.0" && message.pilotLon != "0.0" {
                                Text(String(localized: "pilot_coordinates", comment: "Pilot position coordinates"))
                                    .font(.appCaption)
                            }
                            if let macs = cotViewModel.macIdHistory[message.uid], macs.count > 1 {
                                HStack {
                                    ...
                                    Text(String(localized: "mac_randomizing", comment: "MAC address randomization warning"))
                                        .font(.appCaption)
                                        .foregroundColor(.yellow)
                                }
                            }
