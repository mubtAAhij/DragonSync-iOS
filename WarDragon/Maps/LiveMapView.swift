        ...
        ZStack {
            Map(position: $mapCameraPosition) {
                ...
                ForEach(cotViewModel.alertRings, id: \.id) { ring in
                    ...
                    Annotation(String(localized: "rssi_value", comment: "RSSI signal strength display").replacingOccurrences(of: "{rssi}", with: "\(ring.rssi)"), coordinate: ring.centerCoordinate) {
                        VStack {
                            Text(String(localized: "encrypted_drone", comment: "Label for encrypted drone detection"))
                                .font(.caption)
                            Text(String(localized: "radius_meters", comment: "Radius measurement in meters").replacingOccurrences(of: "{radius}", with: "\(Int(ring.radius))"))
                                .font(.caption)
                ...
                List(uniqueDrones) { message in
                    NavigationLink(destination: DroneDetailView(message: message, flightPath: flightPaths[message.uid]?.map { $0.coordinate } ?? [], cotViewModel: cotViewModel)) {
                        VStack(alignment: .leading) {
                            Text(message.uid)
                                .font(.appHeadline)
                            Text(String(localized: "drone_position", comment: "Drone GPS coordinates").replacingOccurrences(of: "{lat}", with: message.lat).replacingOccurrences(of: "{lon}", with: message.lon))
                                .font(.appCaption)
                            ...
                            if message.pilotLat != "0.0" && message.pilotLon != "0.0" {
                                Text(String(localized: "pilot_position", comment: "Pilot GPS coordinates").replacingOccurrences(of: "{lat}", with: message.pilotLat).replacingOccurrences(of: "{lon}", with: message.pilotLon))
                                    .font(.appCaption)
                            }
                            if let macs = cotViewModel.macIdHistory[message.uid], macs.count > 1 {
                                HStack {
                                    ...
                                    Text(String(localized: "mac_randomizing", comment: "Warning about MAC address randomization").replacingOccurrences(of: "{count}", with: "\(macs.count)"))
                                        .font(.appCaption)
                                        .foregroundColor(.yellow)
                                }
                            }
