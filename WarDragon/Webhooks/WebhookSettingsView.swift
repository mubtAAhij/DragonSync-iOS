    ...
    var body: some View {
        ...
        Form {
            Section(header: Text(String(localized: "webhook_system", comment: "Section header for webhook system settings"))) {
                Toggle(String(localized: "enable_webhooks", comment: "Toggle label to enable webhook functionality"), isOn: .init(
                    get: { settings.webhooksEnabled },
                    set: { settings.updateWebhookSettings(enabled: $0) }
                ))
                
                if settings.webhooksEnabled {
                    Text(String(localized: "webhook_description", comment: "Description explaining webhook functionality"))
                        .font(.caption)
            ...
            if settings.webhooksEnabled {
                Section(header: Text(String(localized: "event_types", comment: "Section header for webhook event types"))) {
                    Text(String(localized: "select_webhook_events", comment: "Description for event selection section"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                    ...
                    HStack {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "disable_all_status_notifications", comment: "Toggle label to disable all status notifications"))
                                .font(.headline)
                            Text(String(localized: "disable_status_notifications_description", comment: "Description for disabling status notifications"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                    ...
                    eventToggleRow(for: .droneDetected, title: String(localized: "drone_detection", comment: "Label for drone detection event type"), description: String(localized: "new_drone_detected", comment: "Description for drone detection event"), icon: "airplane.circle.fill", color: .blue)
...
//                    
                    eventToggleRow(for: .proximityWarning, title: String(localized: "proximity_warning", comment: "Label for proximity warning event type"), description: String(localized: "drone_approaching_threshold", comment: "Description for proximity warning event"), icon: "exclamationmark.triangle.fill", color: .orange)
                    
                    eventToggleRow(for: .systemAlert, title: String(localized: "system_alert", comment: "Label for system alert event type"), description: String(localized: "general_system_warnings", comment: "Description for system alert event"), icon: "exclamationmark.circle.fill", color: .red)
                    
                    eventToggleRow(for: .temperatureAlert, title: String(localized: "temperature_alert", comment: "Label for temperature alert event type"), description: String(localized: "high_temperature_warnings", comment: "Description for temperature alert event"), icon: "thermometer.high", color: .red)
                    
                    eventToggleRow(for: .memoryAlert, title: String(localized: "memory_alert", comment: "Label for memory alert event type"), description: String(localized: "high_memory_usage_warnings", comment: "Description for memory alert event"), icon: "memorychip.fill", color: .yellow)
                    
                    eventToggleRow(for: .cpuAlert, title: String(localized: "cpu_alert", comment: "Label for CPU alert event type"), description: String(localized: "high_cpu_usage_warnings", comment: "Description for CPU alert event"), icon: "cpu.fill", color: .red)
                }
                ...
                Section(header: Text(String(localized: "webhook_services", comment: "Section header for webhook services"))) {
                    if webhookManager.configurations.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            Text(String(localized: "no_webhook_services_configured", comment: "Message when no webhook services are set up"))
                                .foregroundColor(.secondary)
                    ...
                    Button(String(localized: "add_webhook_service", comment: "Button label to add a new webhook service")) {
                ...
                Section(header: Text(String(localized: "recent_deliveries", comment: "Section header for recent webhook deliveries"))) {
                    if webhookManager.recentDeliveries.isEmpty {
                        Text(String(localized: "no_recent_webhook_deliveries", comment: "Message when no recent webhook deliveries exist"))
                            .foregroundColor(.secondary)
                            .italic()
                    } else {
        ...
        .navigationTitle(String(localized: "webhooks", comment: "Navigation title for webhook settings"))
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(String(localized: "events_enabled_count", comment: "Label showing number of events enabled for webhook"))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
        ...
        HStack {
            ...
            VStack(alignment: .trailing, spacing: 2) {
                ...
                if let code = delivery.responseCode {
                    Text(String(localized: "http_status_code", comment: "Label showing HTTP status code"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                if delivery.retryAttempt > 0 {
                    Text(String(localized: "retry_attempt", comment: "Label showing retry attempt number"))
                        .font(.caption)
