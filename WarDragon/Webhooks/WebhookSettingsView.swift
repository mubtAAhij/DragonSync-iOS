        ...
        Form {
            ...
            if settings.webhooksEnabled {
                Section(header: Text(String(localized: "event_types", comment: "Section header for webhook event types"))) {
                    Text(String(localized: "select_webhook_events", comment: "Description for selecting webhook trigger events"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                    ...
                    HStack {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "disable_all_status_notifications", comment: "Label for toggle to disable all status notifications"))
                                .font(.headline)
...
//                    
                    ...
                    eventToggleRow(for: .memoryAlert, title: String(localized: "memory_alert", comment: "Label for memory alert event type"), description: String(localized: "high_memory_warnings", comment: "Description for memory alert event"), icon: "memorychip.fill", color: .yellow)
                    
                    eventToggleRow(for: .cpuAlert, title: String(localized: "cpu_alert", comment: "Label for CPU alert event type"), description: String(localized: "high_cpu_warnings", comment: "Description for CPU alert event"), icon: "cpu.fill", color: .red)
                }
                ...
                Section(header: Text(String(localized: "webhook_services", comment: "Section header for webhook services"))) {
                    if webhookManager.configurations.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            Text(String(localized: "no_webhook_services", comment: "Message when no webhook services are configured"))
                                .foregroundColor(.secondary)
                            Text(String(localized: "add_webhook_services_hint", comment: "Hint text for adding webhook services"))
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    } else {
                    ...
                    Button(String(localized: "add_webhook_service", comment: "Button text to add new webhook service")) {
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(String(localized: "events_enabled_count", comment: "Label showing number of enabled events"))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
        ...
        HStack {
            ...
            VStack(alignment: .trailing, spacing: 2) {
                ...
                if let code = delivery.responseCode {
                    Text(String(localized: "http_status_code", comment: "HTTP status code display"))
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                if delivery.retryAttempt > 0 {
                    Text(String(localized: "retry_attempt", comment: "Retry attempt counter"))
                        .font(.caption)
