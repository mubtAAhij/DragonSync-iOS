...
enum WebhookType: String, CaseIterable, Codable {
    ...
    case custom = String(localized: "custom", comment: "Custom webhook type option")
...
enum WebhookEvent: String, CaseIterable, Codable {
    ...
    var displayName: String {
        switch self {
        case .droneDetected: return String(localized: "drone_detected", comment: "Display name for drone detection event")
    ...
    func toDiscordPayload(username: String?, avatarURL: String?) -> [String: Any] {
        var payload: [String: Any] = [
            "embeds": [[
                ...
                "footer": [
                    "text": String(localized: "wardragon_alert_system", comment: "Footer text for webhook alerts")
                ]
            ]]
        ]
...
class WebhookManager: ObservableObject {
    ...
    private func buildRequest(config: WebhookConfiguration, payload: WebhookPayload) throws -> URLRequest {
        guard let url = URL(string: config.url) else {
            throw NSError(domain: "WebhookManager", code: 1, userInfo: [NSLocalizedDescriptionKey: String(localized: "webhook_invalid_url", comment: "Error message for invalid webhook URL")])
        }
