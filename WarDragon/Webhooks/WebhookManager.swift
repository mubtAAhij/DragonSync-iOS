...
enum WebhookType: String, CaseIterable, Codable {
    ...
    case custom = String(localized: "custom", comment: "Custom webhook type label")
...
class WebhookManager: ObservableObject {
    ...
    private func buildRequest(config: WebhookConfiguration, payload: WebhookPayload) throws -> URLRequest {
        guard let url = URL(string: config.url) else {
            throw NSError(domain: "WebhookManager", code: 1, userInfo: [NSLocalizedDescriptionKey: String(localized: "invalid_url", comment: "Error message when webhook URL is invalid")])
        }
