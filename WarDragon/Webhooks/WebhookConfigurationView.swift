...
struct WebhookConfigurationView: View {
        ...
        NavigationView {
            Form {
                Section(header: Text(String(localized: "basic_configuration", comment: "Section header for basic webhook configuration"))) {
                    TextField(String(localized: "name", comment: "Label for name field"), text: $name)
                    
                    Picker(String(localized: "type", comment: "Label for type picker"), selection: $type) {
                    ...
                    TextField(String(localized: "url", comment: "Label for URL field"), text: $url)
                        .keyboardType(.URL)
                ...
                Section(header: Text(String(localized: "test", comment: "Section header for webhook testing"))) {
                    Button(action: testWebhook) {
                        HStack {
                            ...
                            Text(String(localized: "test_webhook", comment: "Button text for testing webhook"))
                        }
                    }
            ...
            .navigationTitle(config == nil ? String(localized: "add_webhook", comment: "Title for adding new webhook") : String(localized: "edit_webhook", comment: "Title for editing existing webhook"))
    ...
    private var iftttConfigurationSection: some View {
        Section(header: Text(String(localized: "ifttt_configuration", comment: "Section header for IFTTT webhook configuration"))) {
    ...
    private var matrixConfigurationSection: some View {
        Section(header: Text(String(localized: "matrix_configuration", comment: "Section header for Matrix webhook settings"))) {
            TextField(String(localized: "room_id", comment: "Text field label for Matrix room ID"), text: $matrixRoomId)
                .autocapitalization(.none)
            
            SecureField(String(localized: "access_token", comment: "Secure field label for Matrix access token"), text: $matrixAccessToken)
            
            Text(String(localized: "matrix_configuration_help", comment: "Help text explaining Matrix webhook configuration"))
                .font(.caption)
                .foregroundColor(.secondary)
        }
    }
    ...
    private var discordConfigurationSection: some View {
        Section(header: Text(String(localized: "discord_configuration", comment: "Section header for Discord webhook settings"))) {
            TextField(String(localized: "bot_username", comment: "Text field label for Discord bot username"), text: $discordUsername)
            
            TextField(String(localized: "avatar_url_optional", comment: "Text field label for Discord avatar URL"), text: $discordAvatarURL)
                .keyboardType(.URL)
                .autocapitalization(.none)
            
            Text(String(localized: "discord_configuration_help", comment: "Help text explaining Discord webhook configuration"))
                .font(.caption)
                .foregroundColor(.secondary)
        }
    }
    ...
    private var customConfigurationSection: some View {
        Section(header: Text(String(localized: "custom_headers", comment: "Section header for custom HTTP headers"))) {
            ForEach(customHeaders) { header in
                HStack {
                    TextField(String(localized: "header", comment: "Text field placeholder for HTTP header name"), text: .init(
                        get: { header.key },
                        set: { newValue in
                    ...
                    TextField(String(localized: "value", comment: "Text field placeholder for HTTP header value"), text: .init(
                        get: { header.value },
                        set: { newValue in
                            if let index = customHeaders.firstIndex(where: { $0.id == header.id }) {
            ...
            Button(String(localized: "add_header", comment: "Button to add a new HTTP header")) {
    ...
    private func testWebhook() {
        ...
        Task {
            ...
            DispatchQueue.main.async {
                ...
                self.testResult = success ? String(localized: "test_successful", comment: "Success message for webhook test") : String(localized: "test_failed", comment: "Failure message for webhook test")
