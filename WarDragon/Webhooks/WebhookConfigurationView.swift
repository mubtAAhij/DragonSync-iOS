        ...
        NavigationView {
            Form {
                ...
                Section(header: Text(String(localized: "advanced_settings", comment: "Section header for advanced webhook settings"))) {
                    HStack {
                        Text(String(localized: "retry_count", comment: "Label for webhook retry count setting"))
                        Spacer()
                        Stepper("\(retryCount)", value: $retryCount, in: 0...10)
                    }
                    
                    HStack {
                        Text(String(localized: "timeout", comment: "Label for webhook timeout setting"))
                        Spacer()
                ...
                Section(header: Text(String(localized: "test", comment: "Section header for webhook testing"))) {
                    Button(action: testWebhook) {
                        HStack {
                            ...
                            Text(String(localized: "test_webhook", comment: "Button label for testing webhook"))
                        }
                    }
            ...
            .navigationBarItems(
                leading: Button(String(localized: "cancel", comment: "Cancel button in webhook configuration")) {
                    presentationMode.wrappedValue.dismiss()
                },
                trailing: Button(String(localized: "save", comment: "Save button in webhook configuration")) {
                    saveConfiguration()
                }
    ...
    private var customConfigurationSection: some View {
        Section(header: Text(String(localized: "custom_headers", comment: "Section header for custom HTTP headers"))) {
            ForEach(customHeaders) { header in
                HStack {
                    TextField(String(localized: "header", comment: "Placeholder for HTTP header name field"), text: .init(
                        get: { header.key },
                        set: { newValue in
                    ...
                    TextField(String(localized: "value", comment: "Placeholder for HTTP header value field"), text: .init(
                        get: { header.value },
                        set: { newValue in
                            if let index = customHeaders.firstIndex(where: { $0.id == header.id }) {
