...
struct WebhookConfigurationView: View {
    ...
    var body: some View {
        NavigationView {
                ...
                Section(header: Text(String(localized: "test", comment: "Section header for webhook testing"))) {
                    Button(action: testWebhook) {
                        HStack {
                            ...
                            Text(String(localized: "test_webhook", comment: "Button label to test webhook"))
                        }
                    }
            ...
            .navigationTitle(config == nil ? String(localized: "add_webhook", comment: "Navigation title for adding webhook") : String(localized: "edit_webhook", comment: "Navigation title for editing webhook"))
            .navigationBarTitleDisplayMode(.inline)
            .navigationBarItems(
                leading: Button(String(localized: "cancel", comment: "Cancel button in navigation bar")) {
                    presentationMode.wrappedValue.dismiss()
                },
                trailing: Button(String(localized: "save", comment: "Save button in navigation bar")) {
                    saveConfiguration()
                }
    ...
    private var discordConfigurationSection: some View {
        Section(header: Text(String(localized: "discord_configuration", comment: "Section header for Discord webhook settings"))) {
            ...
            TextField(String(localized: "avatar_url_optional", comment: "Label for Discord bot avatar URL field"), text: $discordAvatarURL)
                .keyboardType(.URL)
                .autocapitalization(.none)
    ...
    private var customConfigurationSection: some View {
        Section(header: Text(String(localized: "custom_headers", comment: "Section header for custom HTTP headers"))) {
            ForEach(customHeaders) { header in
                HStack {
                    TextField(String(localized: "header", comment: "Label for HTTP header name field"), text: .init(
                        get: { header.key },
                        set: { newValue in
                    ...
                    TextField(String(localized: "value", comment: "Label for HTTP header value field"), text: .init(
                        get: { header.value },
                        set: { newValue in
                            if let index = customHeaders.firstIndex(where: { $0.id == header.id }) {
            ...
            Button(String(localized: "add_header", comment: "Button to add new HTTP header")) {
