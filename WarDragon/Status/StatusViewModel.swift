        ...
        if memoryUsage > Settings.shared.memoryWarningThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "memory_usage_percentage", comment: "Memory usage alert message with percentage").replacingOccurrences(of: "{percentage}", with: "\(Int(memoryUsage * 100))"),
                    isThresholdAlert: true
                )
            }
        }
        ...
        if lastMessage.antStats.plutoTemp > Settings.shared.plutoTempThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "temperature_celsius", comment: "Temperature alert message with celsius value").replacingOccurrences(of: "{temperature}", with: "\(Int(lastMessage.antStats.plutoTemp))"),
                    isThresholdAlert: true
                )
            }
        }
        ...
        if lastMessage.antStats.zynqTemp > Settings.shared.zynqTempThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "temperature_celsius", comment: "Temperature alert message with celsius value").replacingOccurrences(of: "{temperature}", with: "\(Int(lastMessage.antStats.zynqTemp))"),
                    isThresholdAlert: true
                )
            }
        }
    ...
    private func sendSystemNotification(title: String, message: String, isThresholdAlert: Bool = false) {
        ...
        if Settings.shared.webhooksEnabled && Settings.shared.enabledWebhookEvents.contains(.systemAlert) {
            ...
            if title.contains("Temperature") {
                ...
                event = .memoryAlert
            } else if title.contains(String(localized: "cpu_label", comment: "CPU label in system status")) {
                event = .cpuAlert
            } else {
                event = .systemAlert
            }
