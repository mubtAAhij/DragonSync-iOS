...
extension StatusViewModel {
        ...
        if memoryUsage > Settings.shared.memoryWarningThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "memory_usage_percentage", comment: "Alert message showing current memory usage percentage").replacingOccurrences(of: "{percentage}", with: "\(Int(memoryUsage * 100))"),
                    isThresholdAlert: true
                )
            }
        }
        ...
        if lastMessage.antStats.plutoTemp > Settings.shared.plutoTempThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "temperature_at_degrees", comment: "Alert message showing current temperature").replacingOccurrences(of: "{temperature}", with: "\(Int(lastMessage.antStats.plutoTemp))"),
                    isThresholdAlert: true
                )
            }
        }
        ...
        if lastMessage.antStats.zynqTemp > Settings.shared.zynqTempThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "temperature_at_degrees", comment: "Alert message showing current temperature").replacingOccurrences(of: "{temperature}", with: "\(Int(lastMessage.antStats.zynqTemp))"),
                    isThresholdAlert: true
                )
            }
        }
    ...
    private func checkRegularStatusNotification() {
        ...
        sendSystemNotification(
            ...
            message: String(localized: "system_stats_summary", comment: "System status summary with CPU, memory, and temperature").replacingOccurrences(of: "{cpu}", with: String(format: "%.0f", lastMessage.systemStats.cpuUsage)).replacingOccurrences(of: "{memory}", with: String(format: "%.0f", memoryUsage * 100)).replacingOccurrences(of: "{temperature}", with: "\(Int(lastMessage.systemStats.temperature))"),
    ...
    private func sendSystemNotification(title: String, message: String, isThresholdAlert: Bool = false) {
        ...
        if Settings.shared.webhooksEnabled && Settings.shared.enabledWebhookEvents.contains(.systemAlert) {
            ...
            if title.contains("Temperature") {
                ...
                event = .memoryAlert
            } else if title.contains(String(localized: "cpu_label", comment: "CPU usage label")) {
                event = .cpuAlert
            } else {
                event = .systemAlert
            }
