...
extension StatusViewModel {
    func checkSystemThresholds() {
        ...
        if lastMessage.systemStats.cpuUsage > Settings.shared.cpuWarningThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "cpu_usage_at_percent", comment: "CPU usage percentage message").replacingOccurrences(of: "%d", with: "\(Int(lastMessage.systemStats.cpuUsage))"),
        ...
        if lastMessage.systemStats.temperature > Settings.shared.tempWarningThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "temperature_at_celsius", comment: "Temperature in celsius message").replacingOccurrences(of: "%d", with: "\(Int(lastMessage.systemStats.temperature))"),
                    isThresholdAlert: true
                )
            }
        }
        ...
        if memoryUsage > Settings.shared.memoryWarningThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    ...
                    message: String(localized: "memory_usage_at_percent", comment: "Memory usage percentage message").replacingOccurrences(of: "%d", with: "\(Int(memoryUsage * 100))"),
                    isThresholdAlert: true
                )
            }
        }
        ...
        if lastMessage.antStats.plutoTemp > Settings.shared.plutoTempThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    title: String(localized: "high_pluto_temperature", comment: "Alert title for high Pluto SDR temperature"),
                    message: String(localized: "temperature_at_degrees", comment: "Temperature alert message with temperature value").replacingOccurrences(of: "{temp}", with: "\(Int(lastMessage.antStats.plutoTemp))"),
                    isThresholdAlert: true
                )
            }
        }
        ...
        if lastMessage.antStats.zynqTemp > Settings.shared.zynqTempThreshold {
            if Settings.shared.statusNotificationThresholds {
                sendSystemNotification(
                    title: String(localized: "high_zynq_temperature", comment: "Alert title for high Zynq SDR temperature"),
                    message: String(localized: "temperature_at_degrees", comment: "Temperature alert message with temperature value").replacingOccurrences(of: "{temp}", with: "\(Int(lastMessage.antStats.zynqTemp))"),
                    isThresholdAlert: true
                )
            }
        }
    ...
    private func checkRegularStatusNotification() {
        ...
        sendSystemNotification(
            ...
            message: String(localized: "system_status_details", comment: "System status notification details with CPU, memory, and temperature").replacingOccurrences(of: "{cpu}", with: String(format: "%.0f", lastMessage.systemStats.cpuUsage)).replacingOccurrences(of: "{memory}", with: String(format: "%.0f", memoryUsage * 100)).replacingOccurrences(of: "{temp}", with: "\(Int(lastMessage.systemStats.temperature))"),
    ...
    private func sendSystemNotification(title: String, message: String, isThresholdAlert: Bool = false) {
        ...
        if Settings.shared.webhooksEnabled && Settings.shared.enabledWebhookEvents.contains(.systemAlert) {
            ...
            if title.contains("Temperature") {
                ...
                event = .memoryAlert
            } else if title.contains(String(localized: "cpu_label", comment: "CPU label for system alerts")) {
                event = .cpuAlert
            } else {
                event = .systemAlert
            }
