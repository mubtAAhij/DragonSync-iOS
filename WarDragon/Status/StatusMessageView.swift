...
struct StatusMessageView: View {
    ...
    private var statusHeader: some View {
        VStack(spacing: 8) {
            ...
            HStack {
                Text(String(localized: "last_received", comment: "Label for last received data timestamp"))
                    .font(.system(.caption, design: .monospaced))
                    .foregroundColor(.secondary)
    ...
    private var iPadLayout: some View {
        VStack(spacing: 20) {
            ...
            HStack(alignment: .top, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 16) {
                        CircularGauge(
                            ...
                            maxValue: 100,
                            title: String(localized: "cpu", comment: "CPU usage metric label"),
                            unit: "%",
                        ...
                        CircularGauge(
                            ...
                            maxValue: 100,
                            title: String(localized: "temp", comment: "Temperature metric label"),
                            unit: "°C",
                        ...
                        if message.antStats.plutoTemp > 0 {
                            CircularGauge(
                                ...
                                maxValue: 100,
                                title: String(localized: "pluto", comment: "Pluto hardware temperature label"),
                                unit: "°C",
                        ...
                        if message.antStats.zynqTemp > 0 {
                            CircularGauge(
                                ...
                                maxValue: 100,
                                title: String(localized: "zynq", comment: "ZYNQ hardware temperature label"),
                                unit: "°C",
                ...
                VStack(spacing: 12) {
                    ResourceBar(
                        title: String(localized: "memory", comment: "Memory usage metric label"),
                    ...
                    ResourceBar(
                        title: String(localized: "disk", comment: "Disk usage metric label"),
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack {
                        Text(String(localized: "alt_short", comment: "Altitude abbreviation label"))
                            .font(.system(.caption2, design: .monospaced))
                            .foregroundColor(.secondary)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack(spacing: 16) {
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "cpu", comment: "CPU usage metric label"))
                                .font(.system(.caption2, design: .monospaced))
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "temp", comment: "Temperature metric label"))
                                .font(.system(.caption2, design: .monospaced))
                                .foregroundColor(.secondary)
                    ...
                    HStack(spacing: 16) {
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "memory", comment: "Memory usage metric label"))
                                .font(.system(.caption2, design: .monospaced))
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(String(localized: "disk", comment: "Disk usage metric label"))
                                .font(.system(.caption2, design: .monospaced))
                                .foregroundColor(.secondary)
                        ...
                        if message.antStats.plutoTemp > 0 {
                            VStack(alignment: .leading, spacing: 2) {
                                Text(String(localized: "pluto", comment: "Pluto hardware temperature label"))
                                    .font(.system(.caption2, design: .monospaced))
                                    .foregroundColor(.secondary)
...
struct MemoryDetailView: View {
    ...
    var body: some View {
        NavigationView {
            List {
                Section(String(localized: "memory_usage", comment: "Memory usage chart title")) {
                    MemoryBarView(title: String(localized: "total", comment: "Total memory label"), value: memory.total, total: memory.total, color: .blue)
                    MemoryBarView(title: String(localized: "memory_used", comment: "Memory usage type - used memory"), value: memory.used > 0 ? memory.used : (memory.total - memory.available), total: memory.total, color: .red)
                    MemoryBarView(title: String(localized: "memory_available", comment: "Memory usage type - available memory"), value: memory.available, total: memory.total, color: .green)
                    MemoryBarView(title: String(localized: "memory_free", comment: "Memory usage type - free memory"), value: memory.free, total: memory.total, color: .green)
                    MemoryBarView(title: String(localized: "memory_active", comment: "Memory usage type - active memory"), value: memory.active, total: memory.total, color: .orange)
                    MemoryBarView(title: String(localized: "memory_inactive", comment: "Memory usage type - inactive memory"), value: memory.inactive, total: memory.total, color: .yellow)
                    MemoryBarView(title: String(localized: "memory_buffers", comment: "Memory usage type - buffer memory"), value: memory.buffers, total: memory.total, color: .purple)
                    MemoryBarView(title: String(localized: "memory_cached", comment: "Memory usage type - cached memory"), value: memory.cached, total: memory.total, color: .cyan)
                    MemoryBarView(title: String(localized: "memory_shared", comment: "Memory usage type - shared memory"), value: memory.shared, total: memory.total, color: .pink)
                    MemoryBarView(title: String(localized: "memory_slab", comment: "Memory usage type - slab memory"), value: memory.slab, total: memory.total, color: .indigo)
                }
            }
            .navigationTitle(String(localized: "memory_details", comment: "Title for memory details view"))
...
struct MapDetailView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationTitle(String(localized: "system_location", comment: "Title for system location view"))
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
