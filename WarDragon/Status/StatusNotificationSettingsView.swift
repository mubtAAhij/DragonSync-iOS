...
struct StatusNotificationSettingsView: View {
            ...
            if Settings.shared.webhooksEnabled {
                Section(header: Text("Webhook Status Notifications")) {
                    ...
                    if settings.enabledWebhookEvents.contains(.systemAlert) {
                        Text(String(localized: "webhook_status_notifications_info", comment: "Information about webhook status notifications"))
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
                    ...
                    Section(header: Text(String(localized: "threshold_alerts", comment: "Threshold alerts section header"))) {
                        ...
                        if settings.statusNotificationThresholds {
                            if settings.statusNotificationInterval == .always {
                                Text(String(localized: "threshold_alerts_description_always", comment: "Description for threshold alerts when always mode is enabled"))
                            } else {
    ...
    private func sendTestStatusNotification() {
        ...
        content.body = String(localized: "test_notification_message", comment: "Test notification message")
...
struct StatusSummaryView: View {
    ...
    private var timeUntilNextNotification: String {
        ...
        case .never:
            return "N/A"
        case .always:
            return String(localized: "immediate", comment: "Immediate notification interval")
        case .thresholdOnly:
            return String(localized: "threshold_based", comment: "Threshold-based notification interval")
        default:
            ...
            if timeRemaining <= 0 {
                return String(localized: "ready_to_send", comment: "Notification ready to send status")
            }
