...
struct StatusNotificationSettingsView: View {
            ...
            if Settings.shared.webhooksEnabled {
                Section(header: Text("Webhook Status Notifications")) {
                    ...
                    if settings.enabledWebhookEvents.contains(.systemAlert) {
                        Text(String(localized: "status_webhook_info", comment: "Information about webhook status notifications"))
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
            ...
            if settings.statusNotificationsEnabled {
                ...
                if settings.statusNotificationInterval != .never {
                    Section(header: Text(String(localized: "threshold_alerts", comment: "Threshold alerts section header"))) {
                        Toggle(String(localized: "also_send_threshold_alerts", comment: "Toggle for additional threshold alerts"), isOn: .init(
                        ...
                        if settings.statusNotificationThresholds {
                            if settings.statusNotificationInterval == .always {
                                Text(String(localized: "threshold_alerts_description_always", comment: "Description of threshold alerts for always mode"))
                            } else {
                                Text(String(localized: "threshold_alerts_description_regular", comment: "Description of threshold alerts for regular mode"))
                            }
                        }
                    }
                }
    ...
    private func sendTestStatusNotification() {
        ...
        if Settings.shared.webhooksEnabled {
            let data: [String: Any] = [
                "title": String(localized: "test_status_notification", comment: "Test notification title"),
