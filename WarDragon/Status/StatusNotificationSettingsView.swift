...
struct StatusNotificationSettingsView: View {
    ...
    var body: some View {
                    ...
                    Section(header: Text(String(localized: "threshold_alerts", comment: "Threshold alerts section header"))) {
                        ...
                        if settings.statusNotificationThresholds {
                            if settings.statusNotificationInterval == .always {
                                Text(String(localized: "threshold_alerts_always_description", comment: "Description for threshold alerts when always sending"))
                            } else {
                                Text(String(localized: "threshold_alerts_regular_description", comment: "Description for threshold alerts with regular updates"))
                            }
                        }
                    }
                }
        ...
        .navigationTitle(String(localized: "status_notifications", comment: "Navigation title for status notifications"))
...
struct StatusSummaryView: View {
    ...
    private var timeUntilNextNotification: String {
        ...
        default:
            ...
            if timeRemaining <= 0 {
                return String(localized: "ready_to_send", comment: "Notification ready status")
            }
