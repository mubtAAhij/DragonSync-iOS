...
class CoTViewModel: ObservableObject {
    ...
    private func sendNotification(for message: CoTViewModel.CoTMessage) {
        ...
        content.title = String(localized: "drone_detected_notification_title", comment: "Title for drone detection notification")
        content.body = String(localized: "drone_detected_notification_body", comment: "Body text for drone detection notification").replacingOccurrences(of: "{uid}", with: message.uid).replacingOccurrences(of: "{mac}", with: message.mac ?? "")
