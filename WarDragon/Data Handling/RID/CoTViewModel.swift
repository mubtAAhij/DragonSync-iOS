...
class CoTViewModel: ObservableObject {
    ...
    struct CoTMessage: Identifiable, Equatable {
        ...
        var trackHeading: String? {
            if let course = track_course {
                return String(localized: "course_degrees", comment: "Course heading in degrees format")
            } else if let direction = direction {
                return String(localized: "direction_degrees", comment: "Direction in degrees format")
            }
            return nil
        }
        ...
        var trackSpeedFormatted: String? {
            if let speed = track_speed {
                return String(localized: "speed_meters_per_second", comment: "Speed in meters per second format")
        ...
        var statusDescription: String {
            ...
            let timeSince = Date().timeIntervalSince(lastUpdated)
            if timeSince <= 90 {
                return String(localized: "status_active", comment: "Status indicating drone is currently active")
            } else if timeSince <= 120 {
                return String(localized: "status_aging", comment: "Status indicating drone signal is aging")
            } else {
                return String(localized: "status_stale", comment: "Status indicating drone signal is stale")
            }
        }
        ...
        var formattedAltitude: String? {
            if let altValue = Double(alt), altValue != 0 {
                return String(format: String(localized: "altitude_msl_format", comment: "Altitude in meters above mean sea level format"), altValue)
            }
            return nil
        }
        ...
        var formattedHeight: String? {
            if let heightValue = Double(height ?? ""), heightValue != 0 {
                return String(format: String(localized: "height_agl_format", comment: "Height in meters above ground level format"), heightValue)
            }
            return nil
        }
    ...
    private func sendNotification(for message: CoTViewModel.CoTMessage) {
        ...
        content.title = String(localized: "drone_detected_title", comment: "Notification title for drone detection")
        content.body = String(localized: "drone_detected_body", comment: "Notification body for drone detection with UID and MAC")
...
extension CoTViewModel.CoTMessage {
    ...
    enum ConnectionStatus {
        ...
        var description: String {
            switch self {
            case .connected: return String(localized: "connection_status_connected", comment: "Connection status indicating successful connection")
            case .weak: return String(localized: "connection_status_weak_signal", comment: "Connection status indicating weak signal")
            case .lost: return String(localized: "connection_status_lost", comment: "Connection status indicating lost connection")
