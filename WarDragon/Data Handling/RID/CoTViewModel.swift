...
class CoTViewModel: ObservableObject {
    ...
    struct CoTMessage: Identifiable, Equatable {
        ...
        var trackSpeedFormatted: String? {
            if let speed = track_speed {
                return String(localized: "speed_meters_per_second", comment: "Speed measurement in meters per second").replacingOccurrences(of: "{speed}", with: speed)
            } else if !self.speed.isEmpty && self.speed != "0.0" {
                return String(localized: "speed_meters_per_second", comment: "Speed measurement in meters per second").replacingOccurrences(of: "{speed}", with: self.speed)
            }
            return nil
        }
        ...
        var formattedAltitude: String? {
            if let altValue = Double(alt), altValue != 0 {
                return String(format: String(localized: "altitude_msl_format", comment: "Altitude above mean sea level format"), altValue)
            }
            return nil
        }
        ...
        var formattedHeight: String? {
            if let heightValue = Double(height ?? ""), heightValue != 0 {
                return String(format: String(localized: "height_agl_format", comment: "Height above ground level format"), heightValue)
            }
            return nil
        }
    ...
    private func sendNotification(for message: CoTViewModel.CoTMessage) {
        ...
        content.title = String(localized: "drone_detected", comment: "Notification title for drone detection")
        content.body = String(localized: "drone_detection_details", comment: "Notification body for drone detection").replacingOccurrences(of: "{uid}", with: message.uid).replacingOccurrences(of: "{mac}", with: message.mac ?? "")
...
extension CoTViewModel.CoTMessage {
    ...
    enum ConnectionStatus {
        ...
        var description: String {
            switch self {
            case .connected: return String(localized: "connection_status_connected", comment: "Connected status")
            case .weak: return String(localized: "connection_status_weak_signal", comment: "Weak signal status")
            case .lost: return String(localized: "connection_status_lost", comment: "Connection lost status")
