...
class CoTViewModel: ObservableObject {
    ...
    struct CoTMessage: Identifiable, Equatable {
        ...
        var trackHeading: String? {
            if let course = track_course {
                return String(localized: "degrees_format", defaultValue: "\(course)°", comment: "Format for displaying course in degrees")
            } else if let direction = direction {
                return String(localized: "degrees_format", defaultValue: "\(direction)°", comment: "Format for displaying direction in degrees")
            }
            return nil
        }
        ...
        var trackSpeedFormatted: String? {
            if let speed = track_speed {
                return String(localized: "speed_format_ms", defaultValue: "\(speed) m/s", comment: "Format for displaying speed in meters per second")
            } else if !self.speed.isEmpty && self.speed != "0.0" {
                return String(localized: "speed_format_ms", defaultValue: "\(self.speed) m/s", comment: "Format for displaying speed in meters per second")
            }
            return nil
        }
        ...
        var statusDescription: String {
            ...
            let timeSince = Date().timeIntervalSince(lastUpdated)
            if timeSince <= 90 {
                return String(localized: "status_active", defaultValue: "Active", comment: "Status indicating drone is active")
            } else if timeSince <= 120 {
                return String(localized: "status_aging", defaultValue: "Aging", comment: "Status indicating drone signal is aging")
            } else {
                return String(localized: "status_stale", defaultValue: "Stale", comment: "Status indicating drone signal is stale")
            }
        }
        ...
        var formattedAltitude: String? {
            if let altValue = Double(alt), altValue != 0 {
                return String(format: String(localized: "altitude_msl_format", defaultValue: "%.1f m MSL", comment: "Format for displaying altitude in meters above mean sea level"), altValue)
            }
            return nil
        }
        ...
        var formattedHeight: String? {
            if let heightValue = Double(height ?? ""), heightValue != 0 {
                return String(format: String(localized: "height_agl_format", defaultValue: "%.1f m AGL", comment: "Format for displaying height in meters above ground level"), heightValue)
            }
            return nil
        }
    ...
    private func sendNotification(for message: CoTViewModel.CoTMessage) {
        ...
        content.title = String(localized: "notification_drone_detected", defaultValue: "Drone Detected", comment: "Notification title when drone is detected")
        content.body = String(localized: "notification_drone_details", defaultValue: "From: \(message.uid)\nMAC: \(message.mac ?? "")", comment: "Notification body showing drone details")
...
extension CoTViewModel.CoTMessage {
    ...
    enum ConnectionStatus {
        ...
        var description: String {
            switch self {
            case .connected: return String(localized: "connection_status_connected", defaultValue: "Connected", comment: "Status indicating device is connected")
            case .weak: return String(localized: "connection_status_weak_signal", defaultValue: "Weak Signal", comment: "Status indicating weak signal connection")
            case .lost: return String(localized: "connection_status_lost", defaultValue: "Connection Lost", comment: "Status indicating connection has been lost")
